...
第二步，一条 state 记录包含了 url、title 和 content 属性，在 popstate 事件中可以 获取到这个 state 对象，我们可以使用 content 来传递数据。
...
## Vue路由跳转方式有哪几种？
1. router-link
router-lin组件支持用户在具有路由功能的应用中导航。通过to属性指定目标地址。
...
- params只能用name来引入路由，不能用path，否则接收不到参数的；query用path来引入

// 传递
this.$router.push({name: './xxx', params: {xx:xxx}})
this.$router.push({path: './xxx', query: {xx:xxx}})

- params类似post, 跳转之后页面 url 后面不会拼接参数, 但是刷新页面id 会消失，params传值相对安全一些
